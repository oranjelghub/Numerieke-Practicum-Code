function [A,b] = stelselsplineint(t, f, tbefore, tafter)
    ...
    for i = 1:L
        A(i+1,i:i+2) = [ppval(bspline([knpvec(i),knpvec(i+1),knpvec(i+2),knpvec(i+3),knpvec(i+4)]),t(i))
    ...
    A(1,1:3) = [ppval(fnder(bspline([knpvec(1),knpvec(2),knpvec(3),knpvec(4),knpvec(5)]),2),t(1))
                ppval(fnder(bspline([knpvec(2),knpvec(3),knpvec(4),knpvec(5),knpvec(6)]),2),t(1))
                ppval(fnder(bspline([knpvec(3),knpvec(4),knpvec(5),knpvec(6),knpvec(7)]),2),t(1))];

    A(L+2,L:L+2) = [ppval(fnder(bspline([knpvec(L),knpvec(L+1),knpvec(L+2),knpvec(L+3),knpvec(L+4)]),2),t(L))
